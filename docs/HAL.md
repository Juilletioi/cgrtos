...
版本：与内核同步（当前 **4.7.0**）  
头文件：`hal/hal.h`（用户 API）· `hal/hal_board.h`（板级 MMIO）  
实现：`hal/hal_core.c` + `hal/hal_compat.c` + `hal/hal_printf.c` + `arch/riscv/*.c`（纯驱动）  
相关：`docs/ARCHITECTURE.md` · `docs/USER_GUIDE.md` · `./scripts/cgrtos.sh sdk`

---

## 1. 目标与动机

在引入 HAL 之前，UART / CLINT / PLIC / IPI 的基址与访问散落在 `arch/riscv/*.c`，应用通过 `cgrtos_uart_*` 等直接触达硬件。单板可用，但存在：

1. **可移植性差**：换 SoC 要改多处硬编码。
2. **契约不清**：ISR / 多核下哪些 API 安全缺少统一说明。
3. **并发不安全**：多核同时 `printf` 会把输出字符交错。

HAL 将「稳定的用户接口」与「板级 MMIO + 驱动 ops」分离，并补齐错误码、锁与文档契约。历史 `cgrtos_*` 外设 API 保留为薄封装，旧代码无需改动。
...
```
  App / Kernel / Tests
       ...
       v
  HAL 用户 API（hal/hal.h + hal_core / hal_compat / hal_printf）
       |  错误码 · 控制台锁 · IRQC 配置锁 · 类别缓存
       |  boot：drv_*_device() → hal_device_register → hal_*_init
       v
  Board drivers（arch/riscv/*.c：只实现 ops + MMIO）
       |  MMIO 只允许来自 hal/hal_board.h；**禁止**调用 hal_*
       v
  硬件（Nuclei UX900 / evalsoc）

  Trap / 底层 ISR ──直调──> 驱动内部 *_hw_*（禁止再进 hal_irqc_* 等）
```

**依赖方向必须牢记：HAL 调用驱动；驱动与 trap 不得调用 HAL 用户 API。**

启动：`cgrtos_init()` → `hal_board_init()`：

1. `drv_cpu/uart/plic/clint/ipi_device()` 取得静态描述符并 `hal_device_register`  
2. 按序初始化：CPU → Console → IRQC → Timer → IPI  
3. **冻结注册表**（此后 `hal_device_register` 返回 `HAL_ERR_STATE`）
